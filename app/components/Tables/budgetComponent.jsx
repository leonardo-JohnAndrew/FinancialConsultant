...
const Row = React.memo(function Row({
  ...
  return (
    <>
      <tr
        style={{ fontWeight: isMain ? "bold" : "normal" }}
        className="text-center border border-dotted"
      >
        <td style={{ paddingLeft: `${level * 20}px` }}>{item.code}</td>
        ...
        <td className="border-r-2 text-left">
          {!isMain && (
            <input
              ...
              style={{
                width: `${String(v.cumulative_qty || "").length + 3}ch`,
              }}
              className="border border-gray-300 px-1 bg-gray-100"
            />
          )}
    ...
    <div className="overflow-x-auto">
      ...
      >
        <thead>
          {/* GROUP HEADERS */}
          <tr>
            <th className="bg-black text-white border border-white" rowSpan="2">
              SN
            </th>
            <th
              className="bg-black text-white border-l-2 border-l-white "
              rowSpan="2"
            >
              Description
            </th>
            <th className="bg-black border-none" rowSpan={2}></th>
            <th className="bg-black text-white border border-white" colSpan="4">
              Approved
            </th>
            <th className="bg-black text-white border border-white" colSpan="3">
              Modified Cost
            </th>
            <th className="bg-black text-white border border-white" colSpan="2">
              Previous Claimed
            </th>
            <th className="bg-black text-white border border-white" colSpan="2">
              This Month
        ...
        <tbody>
          ...
          <tr className="text-center border-t-2">
            ...
            <td className="border-r px-1">
              {formatMoney(
                parseFloat(total?.approved_amount || 0),
                "PHP",
                "en-PH",
              )}
            </td>
            <td colSpan="4"></td>
            <td className="border-r px-1">
              {formatMoney(parseFloat(total.prev_amount || 0), "PHP", "en-PH")}
            </td>
            <td></td>
            <td className="border-r px-1 ">
              {formatMoney(parseFloat(total.month_amount || 0), "PHP", "en-PH")}
            </td>
            <td></td>
            <td className="border-r px-1">
              {formatMoney(
                parseFloat(total.cumulative_amount || 0),
                "PHP",
                "en-PH",
              )}
            </td>
            ...
            <td className="border-r px-1">
              {formatMoney(
                parseFloat(total.remaining_amount || 0),
                "PHP",
                "en-PH",
              )}
            </td>
