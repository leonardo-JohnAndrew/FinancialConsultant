...
import { accountingNofication } from "@/functions/notification";
...
export async function POST(request) {
  ...
  for (const item of body.purchaseItem) {
    ...
    if (missing.length > 0) {
      missingFields[`purchaseItem_${body.purchaseItem.indexOf(item) + 1}`] =
        missing;
    }
  }
  ...
  try {
    ...
    const userDprt = await User.findByPk(body.purchaseItem[0]?.UserID, {
      attributes: ["department", "lastname", "firstname"],
    });
    if (!userDprt) {
      return NextResponse.json(
    ...
    const items =
      await sequelize.models.purchaseItems.bulkCreate(purchaseItemsData);
    // check if
