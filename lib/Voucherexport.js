...
/** Load image from URL or local path → Buffer */
async function loadImage(src) {
  ...
  // Treat any non-URL src as a web-relative path under /public,
  // regardless of leading slash.
...
async function fillTemplate(
  ...
  setCell(ws, mm, "I6", d1.d1);
  setCell(ws, mm, "J6", d1.d2);
  setCell(
    ws,
    mm,
    "L8",
  ...
  if (isDouble && item2) {
    ...
    setCell(ws, mm, "I30", d2.d1);
    setCell(ws, mm, "J30", d2.d2);
  ...
  // ── Cheque attachment at A50 ──
  if (insertAttachment && check.cheque_attachment) {
    try {
      const src = check.cheque_attachment;
      const imgBuffer = await loadImage(src);
      const imgId = wb.addImage({
        buffer: imgBuffer,
        extension: getImageExt(src),
      });
      ws.addImage(imgId, {
        tl: { col: 0, row: 49 },
      ...
      console.warn("Could not insert cheque attachment:", e.message);
    }
  }
...
export async function buildVoucherWorkbook(checkData) {
  ...
  // Total sheets na mabubuo
  const totalSheets = pairs + remainder;

  const sheets = [];
  let sheetIndex = 0; // 1-based counter para malaman kung huli na

  for (let i = 0; i < pairs; i++) {
    sheetIndex++;
    const tplName =
      i === 0 && isClaimable ?
        "Double_claimable_Voucher.xlsx"
      : "Double_Voucher.xlsx";

    const isLastSheet = sheetIndex === totalSheets;
    ...
    const buffer = await fillTemplate(
      ...
      checkData,
      true,
      isLastSheet, // <-- insertAttachment flag
    );
